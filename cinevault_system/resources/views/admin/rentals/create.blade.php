        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.rentals.store') }}" id="rental-form">
                    ...
                    <select class="form-select" name="movie_id" required id="movie-select" onchange="updatePrice()">
                        ...
                        @foreach($movies as $movie)
                            <option value="{{ $movie->id }}"
                                ...
                                <i class="{{ $movie->poster_icon }}"></i> {{ $movie->title }} — ₱{{ number_format($movie->price_per_day) }}/day
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div style="padding:14px; background:rgba(200,160,74,.07); border:1px solid rgba(200,160,74,.18); border-radius:6px; margin-bottom:16px;">
                    ...
                    <div style="display:flex; justify-content:space-between; font-size:16px; font-weight:700; margin-top:8px; padding-top:8px; border-top:1px solid rgba(200,160,74,.18);">
