...
@section('content')
{{-- Tabs --}}
<div style="display:flex; border-bottom:1px solid var(--border); margin-bottom:20px; gap:0;">
    @foreach(['active'=>'Active Rentals','history'=>'Return History','payments'=>'Payment Records'] as $key=>$label)
        <a href="{{ request()->fullUrlWithQuery(['tab'=>$key]) }}"
           style="padding:9px 18px; font-size:13px; color:{{ $tab===$key ? 'var(--gold)' : 'var(--text3)' }}; border-bottom:2px solid {{ $tab===$key ? 'var(--gold)' : 'transparent' }}; text-decoration:none; transition:all .15s; margin-bottom:-1px;">
            {{ $label }}
        </a>
    @endforeach
</div>
...
<div class="card" style="overflow-x:auto;">
    <table class="data-table">
        ...
        <tbody>
                ...
                <tr>
                    <td>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <span style="font-size:20px;">{{ $rental->movie->poster_emoji }}</span>
                            <div>
                    ...
                    <td>
                        <span style="color:{{ $rental->isOverdue() ? 'var(--red)' : 'var(--text)' }};">
                            ...
                            @if($rental->isOverdue()) <span class="badge badge-red" style="margin-left:4px;">Overdue</span> @endif
                        </span>
                    </td>
                    <td style="font-weight:700; color:var(--gold);">₱{{ number_format($rental->total_amount) }}</td>
                    <td>
                        @php $pm = $rental->payment_method; @endphp
                        <span class="badge {{ $pm==='gcash' ? 'badge-blue' : ($pm==='card' ? 'badge-purple' : 'badge-green') }}">
                            {{ strtoupper($pm) }}
                        </span>
                    ...
                    @if($tab === 'active')
                        <td>
                            @if(in_array($rental->status, ['active','overdue']))
                                <form method="POST" action="{{ route('admin.rentals.return', $rental) }}" onsubmit="return confirm('Mark as returned?')">
                                    @csrf @method('PATCH')
            ...
            @empty
                <tr><td colspan="9" style="text-align:center; padding:40px; color:var(--text3);">No rentals found.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
