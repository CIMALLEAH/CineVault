...
<div style="display:flex; align-items:center; gap:10px; margin-bottom:16px; flex-wrap:wrap;">
    <form method="GET" style="display:flex; gap:8px; flex-wrap:wrap; flex:1;">
        ...
        <select name="status" class="form-select" style="width:auto;" onchange="this.form.submit()">
            ...
            <option value="rented"    {{ request('status') === 'rented'    ? 'selected' : '' }}>Rented Out</option>
        </select>
...
<div style="display:grid; grid-template-columns:repeat(auto-fill,minmax(180px,1fr)); gap:16px;">
    @forelse($movies as $movie)
        ...
        @endphp
        <div class="card" style="overflow:hidden; transition:transform .2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform=''">
            ...
            <div style="height:210px; position:relative; background:linear-gradient(145deg,var(--bg3),var(--bg4)); display:flex; align-items:center; justify-content:center; font-size:56px; overflow:hidden;">
                ...
                {{-- Availability badge --}}
                <span class="badge {{ $canRent ? 'badge-green' : 'badge-red' }}" style="position:absolute; top:8px; left:8px;">
            ...
            <div style="padding:12px;">
                <div style="font-size:13px; font-weight:600; margin-bottom:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="{{ $movie->title }}">{{ $movie->title }}</div>
