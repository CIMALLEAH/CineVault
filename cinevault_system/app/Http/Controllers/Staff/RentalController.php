...
class RentalController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        AuditLog::write('RENTAL_CREATED',
            "Staff " . auth()->user()->name . " rented \"{$movie->title}\" to {$rental->customer_name}.",
            auth()->id(), Rental::class, $rental->id);
 
        return redirect()->route('staff.rentals.index')
                         ->with('success', "Rental confirmed! Total: ₱{$rental->total_amount}");
    }
    ...
    public function returnMovie(Rental $rental)
    {
        ...
        $rental->update(['status' => 'returned', 'returned_date' => now()->toDateString()]);
